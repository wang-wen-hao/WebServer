...
void HttpData::reset() {
  LOG << CurrentThread::tid() << " HttpData::reset()";
  // inBuffer_.clear();
  fileName_.clear();
  path_.clear();
  nowReadPos_ = 0;
...
void HttpData::handleRead() {
  LOG << CurrentThread::tid() << " HttpData::handleRead()";
...
void HttpData::handleWrite() {
  LOG << CurrentThread::tid() << " HttpData::handleWrite()";
  ...
  if (!error_ && connectionState_ == H_CONNECTED) {
    if (events_ != 0) {
      ...
      if ((events_ & EPOLLIN) && (events_ & EPOLLOUT)) {
        ...
        events_ |= EPOLLOUT;
        LOG << "if event is in and out, change event to out";
      }
      ...
      LOG << "keepAlive is true, events_ |= in | ET";
      ...
      LOG << "else, events_ |= in | ET, timeout = alive_time >> 1";
      loop_->updatePoller(channel_, timeout);
    }
  ...
  if (pos < 0) {
    ...
    if (_pos < 0)
      ...
      if (_pos - pos > 1) {
        ...
        fileName_ = "/home/wwh/Documents/WebServerProjects/WebServer/index.html";
    }
    pos = _pos;
  }
...
void HttpData::handleError(int fd, int err_num, string short_msg) {
  LOG << CurrentThread::tid() << " HttpData::handleError";
  short_msg = " " + short_msg;
  char send_buff[4096];
...
void HttpData::handleClose() {
  LOG << CurrentThread::tid() << " HttpData::handleClose()";
